...
/** The GraphQL query used to get organisms for trait search. */
export const getTraitOrganismsQuery = `
  query OganismsQuery($taxonId: Int, $abbreviation: String, $name: String, $genus: String, $species: String, $page: Int, $pageSize: Int) {
  organisms(taxonId: $taxonId, abbreviation: $abbreviation, name: $name, genus: $genus, species: $species, page: $page, pageSize: $pageSize) {
      results {
        genus
 ...
 * @returns {Promise} A `Promise` that resolves to the result of the GraphQL query.
 */
export function getTraitOrganisms(queryData={}, pageData={}, options={}) {
  const {taxonId, abbreviation, name, genus} = queryData;
  const {page, pageSize} = pageData;
  const variables = {
    taxonId,
    abbreviation,
...
/**
 * Converts GraphQL `OrganismsResults` into the `*SearchFormData` used by the `LisTraitAssociationSearchElement` (`<lis-trait-association-search-element>`) Web Component.
 * @param {object} data - An object containing the data portional of the GraphQL query HTTP response.
 * @returns {object} A `*SearchFormData` object.
 */
export function traitOrganismsDataToFormData(data) {
  // bin the strains by genus then species
  const binnedFormData = {};
  ...
  const genuses =
    Object.entries(binnedFormData).map(([genus, binnedSpecies]) => {
      const species =
        Object.entries(binnedSpecies).map(([species]) => {
          return {species};
        });
      return {genus, species};
    });
 ...
 */
export function getTraitOrganismsFormDataFunction(queryData={}, pageData={}, options={}) {
  return getTraitOrganisms(queryData, pageData, options)
  .then(({data}) => traitOrganismsDataToFormData(data));
}